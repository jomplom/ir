...
let optimize_target ~triple ~datalayout passes m =
  ...
  | triple ->
      ...
      Llvm_target.TargetMachine.add_analysis_passes passes target;
      Llvm_passmgr_builder.populate_module_pass_manager passes m

let optimize ~level ~lto ~triple ~datalayout passes m =
  let pm = Llvm.PassManager.create () in
  let b = Llvm_passmgr_builder.create () in
  optimize_target ~triple ~datalayout passes m;
  ...
  List.iter (fun pass -> Llvm.PassManager.add pass pm) passes;
...
let main level lto triple datalayout passes file =
  let buf = Llvm.MemoryBuffer.of_file file in
  let m = Llvm_irreader.parse_ir ctxt buf in
  begin match level with
  | (0 | 1 | 2 | 3) as level -> optimize ~level ~lto ~triple ~datalayout passes m
  | -1 -> ()
...
let term =
  ...
  let doc_passes = "Additional optimization passes to be applied." in
  ...
  Cmdliner.Arg.(value & opt_all string [] & info ~doc:doc_passes ["passes"]) $
  Cmdliner.Arg.(required & pos 0 (some file) None & info ~docv:"FILE" [])

let info =
  Cmdliner.Term.info
    ~doc:"tiny LLVM-IR optimizer"
    ~man:[`P "takes a LLVM-IR file as input and dumps \
              the optimized LLVM-IR module in the std output."]
